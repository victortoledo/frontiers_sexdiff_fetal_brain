...
gene.list.randomization = function(your.list, list.of.interest, background, B) { 
  ...
  for(i in 1:B) {
    r.list.unique = as.vector(sample(background, size = 2*C, replace = T))
...
to database as the actual expression results.", sep = ""), file="")
  ...
  density(random.gene.ns)-> table_density
  as.data.frame(cbind(table_density$x,table_density$y))-> table_density2
  write.table(table_density2, paste(interestfile, "MSET_OUTPUT_TABLE.txt", sep=""), sep="\t", row.names = F, col.names = F, quote=F)
  #write(paste(density(random.gene.ns)), file="")
  write("============================================================================", file="")
